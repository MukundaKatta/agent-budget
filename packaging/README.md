...
Distribution-specific packaging files for `agent-budget`. Each subdirectory contains a recipe + a `SUBMIT.md` walking through the submission flow.

| Directory | Target | Status | Submit-side action required |
|---|---|---|---|
| `aur/` | Arch User Repository | drafted | Arch account + SSH key. `git push` to `aur@aur.archlinux.org:python-agent-budget.git` |
