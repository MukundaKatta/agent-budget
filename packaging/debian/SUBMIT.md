...
wget -O python-agent-budget_0.1.0.orig.tar.gz \
    https://github.com/MukundaKatta/agent-budget/releases/download/v0.1.0/agent_budget-0.1.0.tar.gz

# 2. Unpack and copy the staged debian/ in:
tar xzf python-agent-budget_0.1.0.orig.tar.gz
cd agent_budget-0.1.0
...
cd ..
dput ppa:mukundakatta/agent-stack python-agent-budget_0.1.0-1_source.changes
```
...
sudo apt update
sudo apt install python3-agent-budget
```
