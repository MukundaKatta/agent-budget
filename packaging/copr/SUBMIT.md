...
copr-cli create agent-stack \
    ...
    --instructions "dnf copr enable mukundakatta/agent-stack && dnf install python3-agent-budget" \
...
mock -r fedora-rawhide-x86_64 --buildsrpm \
    --spec packaging/copr/python-agent-budget.spec \
    --resultdir /tmp/srpm/

# 3. Upload + build:
copr-cli build agent-stack /tmp/srpm/python-agent-budget-0.1.0-1.fc41.src.rpm
```
...
sudo dnf install python3-agent-budget
```
